...
func TestParsingArrayLiterals(t *testing.T) {
	...
	err := testIntegerLiteral(t, array.Elements[0], 1)
	if err != nil {
		t.Errorf("[Error] %v", err)
	}
	...
	err = testInfixExpression(t, array.Elements[1], 2, "*", 2)
	if err != nil {
		t.Errorf("[Error] %v", err)
	}

	err = testInfixExpression(t, array.Elements[2], 3, "+", 3)
	if err != nil {
		t.Errorf("[Error] %v", err)
	}
}
...
func TestParsingIndexExpression(t *testing.T) {
	...
	err := testIdentifier(t, indexExp.Left, "myArray")
	if err != nil {
		t.Errorf("not myArray %v\n", indexExp.Left)
	}

	err = testInfixExpression(t, indexExp.Index, 1, "+", 1)
	if err != nil {
		t.Errorf("not 1+1 %v", indexExp.Index)
	}
}
...
func TestParsingHashLiteralsStringKeys(t *testing.T) {
	...
	for key, value := range hash.Pairs {
		...
		err := testIntegerLiteral(t, value, expectedValue)
		if err != nil {
			t.Errorf("[ERROR] %v", err)
		}
	}
...
func TestParsingHashLiteralsWithExpressions(t *testing.T) {
	...
	tests := map[string]func(ast.Expression){
		"one": func(e ast.Expression) {
			err := testInfixExpression(t, e, 0, "+", 1)
			if err != nil {
				t.Errorf("[ERROR] %v", err)
			}
		},
		"two": func(e ast.Expression) {
			err := testInfixExpression(t, e, 10, "-", 8)
			if err != nil {
				t.Errorf("[ERROR] %v", err)
			}
		},
		"three": func(e ast.Expression) {
			err := testInfixExpression(t, e, 15, "/", 5)
			if err != nil {
				t.Errorf("[ERROR] %v", err)
			}
		},
